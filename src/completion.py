    ...
    if status is CompletionResult.OK or status is CompletionResult.MODERATION_FLAGGED:
        ...
        if not reply_text:
            sent_message = await thread.send(
                embed=discord.Embed(
                    description=f"**В ответ закрились враги** - результат пуст",
        ...
        if status is CompletionResult.MODERATION_FLAGGED:
            ...
            await thread.send(
                embed=discord.Embed(
                    description=f"⚠️ **Нити сущего встревожены.**",
                    color=discord.Color.yellow(),
                )
            )
        ...
        await thread.send(
            embed=discord.Embed(
                description=f"❌ **Что-то нарушило тонкую нить плетения...**",
        ...
        await thread.send(
            embed=discord.Embed(
                description=f"**В ответ закрились враги** - {status_text}",
                color=discord.Color.yellow(),
            )
        )
    else:
        await thread.send(
            embed=discord.Embed(
                description=f"**Ошибка сущего** - {status_text}",
                color=discord.Color.yellow(),
            )
        )
